  ...
  while(1)
  {
    ...
    else if (strcmp(msg_cmd.command,"client_exit") == 0)
    {
      ...
      *(user->order)--;
      ...
      if ((temp_fp = fopen("SYS_LOG.txt","a")) != NULL)
      {
	printf("用户: %s 注销登陆,%s\n",user->username,real_time);
	fprintf(temp_fp,"用户: %s 注销登陆,%s\n",user->username,real_time);
	fclose(temp_fp);
      }
      pthread_exit(0);
    }
...
else
{
  *result = 'y';
  ...
  printf("用户: %s 注册成功,%s\n",client.name,real_time);
  fprintf(fp_log,"用户: %s 注册成功,%s\n",client.name,real_time);
  fclose(fp_log);
  send(fd, "success",strlen("success"), 0);
  fclose(fp);
}
*result = 'y';
send(fd, "success",strlen("success") + 1, 0);
...
void authorise_client(int fd, USER *user)//用户注册函数,客户端
{
  ...
  while(strcmp(status,"fail") == 0)
  {
   ...
   receive(status, fd, sizeof(status));
   if (strcmp(status,"success") == 0 || strcmp(status,"uccess") == 0)
   {
...
void *show_clients_server(void *arg)//在服务端查看正在使用系统的用户的函数
{
  STUD *q = (STUD *)arg;
  STUD *p;
  char real_time[64];
